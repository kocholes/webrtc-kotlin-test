...
//            val g = this.convertFloatsToDoubles(event.values.clone())
//
//            //Normalise
//            val norm = Math.sqrt(
...
//            sendMessage("rot|${Math.toDegrees(Math.acos(event.values[0].toDouble()))},${Math.toDegrees(event.values[1].toDouble())},${Math.toDegrees(event.values[2].toDouble())}")
//            val orientation = getOrientationFromRotationVector(event)
//            sendMessage("rot|${orientation}")
//        }
//    }
